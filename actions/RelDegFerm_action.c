...
double dLagrangian( site *s ){
  double tmp,dL;
  tmp = (*s).y * (*s).y;
  dL = params.S1 * tmp * tmp - params.S2 * (*s).b * (*s).b / tmp;
  return(dL);
}
...
void dF_dbXy( double *dF, site *s ){
  ...
  dF[1] = 0.0;
  dF[2] = params.S1 * 4.0 * tmp * (*s).y + 
    params.S2 * 2.0 * (*s).b * (*s).b / ( tmp * (*s).y );
  return;
}
...
void d2F_dbXy2( double *d2F, site *s ){
  ...
  d2F[1] = 0.0;
  d2F[2] = params.S2 * 4.0 * (*s).b / ( tmp * (*s).y );
  d2F[3] = 0.0;
  d2F[4] = 0.0;
  d2F[5] = params.S1 * 12.0 * tmp - 
    params.S2 * 6.0 * (*s).b * (*s).b / ( tmp * tmp );
  return;
}
...
double dS_ddmuphi( site *s, int i, int mu ){
  ...
  tmp = (*s).y * (*s).y;
  /*  dS/d(d_mu phi_i) ~ [-4 y^4 - 4 C (b/y)^2] (B^-1)_ij (d_mu phi_j)  */
  dSdd = 0.0;
  for(j=0;j<3;j++) dSdd += (*s).Binv[i][j] * (*s).dphi[j][mu];
  dSdd *= -4.0 * ( params.S1 * tmp * tmp + params.S2 * (*s).b * (*s).b / tmp );
...
double dS_ddmupsi( site *s, int mu ){
  ...
  tmp = (*s).y * (*s).y;
  /*  dS/d(d_mu psi) ~ u_mu (4 y^3 + 2 C b^2 y^-3)  */
  dSdd = (*s).uu[mu] * ( 4.0 * params.S1 * tmp * (*s).y + 
			 2.0 * params.S2 * (*s).b * (*s).b / 
			 ( tmp * (*s).y ) );
...
void init_bound( ){
  register int is,x,t,i,j,k,l,js;
  register site *s,*sn;
  register double cp;
  int dr[4];
  /*
  ...
  */
  if( params.bc == TIME_MixBC ){
	...
	latt_bound[k].phi[i] = (*sn).phi[i];
      for(i=0;i<4;i++) latt_bound[k].uu[i] = 0.0;
      k++;
    }
    dr[3] = 1;
    t = params.Nt - 1;
