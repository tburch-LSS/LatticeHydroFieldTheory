/*  "alternative" relativistic, degenerate fermion action: 
    ...
    dS = d^4x [S_1 X^2 - S_2 b^2/X]  ; (xi^2=0)  */
...
double dLagrangian( site *s ){
  double dL;
  dL = params.S1 * (*s).X * (*s).X - params.S2 * (*s).b * (*s).b / (*s).X;
  return(dL);
}
...
void dF_dbXy( double *dF, site *s ){
  dF[0] = -params.S2 * 2.0 * (*s).b / (*s).X;
  dF[1] = params.S1 * 2.0 * (*s).X + 
    params.S2 * (*s).b * (*s).b / ( (*s).X * (*s).X );
  dF[2] = 0.0;
  return;
}
...
void d2F_dbXy2( double *d2F, site *s ){
  double tmp;
  tmp = (*s).X * (*s).X;
  d2F[0] = -params.S2 * 2.0 / (*s).X;
  d2F[1] = params.S2 * 2.0 * (*s).b / tmp;
  d2F[2] = 0.0;
  d2F[3] = params.S1 * 2.0 - 
    params.S2 * 2.0 * (*s).b * (*s).b / ( tmp * (*s).X );
  d2F[4] = 0.0;
  d2F[5] = 0.0;
  return;
}
...
double dS_ddmuphi( site *s, int i, int mu ){
  ...
  /*  dS/d(d_mu phi_i) ~ -2 (b^2/X) (B^-1)_ij (d_mu phi_j)  */
  dSdd = 0.0;
  for(j=0;j<3;j++) dSdd += (*s).Binv[i][j] * (*s).dphi[j][mu];
  dSdd *= -2.0 * params.S2 * (*s).b * (*s).b / (*s).X;
...
double dS_ddmupsi( site *s, int mu ){
  ...
  /*  dS/d(d_mu psi) ~ 2 (d_mu psi) (2X + C b^2 X^-2)  */
  dSdd = 2.0 * (*s).dpsi[mu] * ( params.S1 * 2.0 * (*s).X + 
				 params.S2 * (*s).b * (*s).b / 
				 ( (*s).X * (*s).X ) );
